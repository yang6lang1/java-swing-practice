...
public class RollerCoaster {
	...
	private int[][] table = new int[T][1000];	//pre-processing data: code optimization
	private int front = 0,end = N-1,num_group_inRC=0;
	//private String fileName="inputFile.txt";
	private String fileName="C-small-practice.txt";
	//private String fileName="C-large-practice.txt";
	...
	public RollerCoaster(){
	    ...
	    fileInput = new Scanner(fileInBR);
	     ...
	     for(int i =0; i <T;i++){
		     ...
		     front = 0;end = N-1;num_group_inRC=0;
		     ...
		     for(int rounds=0;rounds<R;rounds++){
		    	 ...
		    	 if(N==1){//special case: only one group
			    	 ...
			    	 while(left_seats>0 && num_group_inRC<N){
			    		 ...
			    		 if(front == end && left_seats>=g[front] && num_group_inRC<N){
			    			 income = income + price*g[front];
			    			 left_seats = left_seats - g[front];
			    			 num_group_inRC++;
			    			 break;
